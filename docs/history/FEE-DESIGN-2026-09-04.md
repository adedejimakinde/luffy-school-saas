...
Everything here follows from nine domain answers. Where a decision was made *by
the school* rather than by the code, it is marked **ruled**; where the code
chose, the reasoning is given and can be overturned.
...
> `ReportCardSettings` and `SessionSettings`. Rejected: `SessionSettings` earned
> its own row because it holds arithmetic that `results.sessions` owns
> end-to-end, not because settings split by subject, and one boolean plus one
> string does not earn a table. This is the most overturnable decision in the
> document.
...
Flagged as a distinct decision from "staff always see a withheld card": the gate
not applying to staff claims is ruled, while granting the bursar the card
endpoint at all is a widening, and a narrower alternative exists (a staff screen
that shows only *that* a card exists and is withheld). Recommended as written,
because a bursar lifting a withholding wants to see the document they are
releasing.
...
1. **Revoking a concession leaves no log.** Setting `is_active` false records
   *when* through `updated_at` and never who or why. This follows the operating
   ...
   rule's own logic an absence wants a log. It is a lesser question than a
   withheld card and it is a real one. Candidate issue rather than a silent gap.
2. **A third settings singleton** instead of two new columns on
   `ReportCardSettings`, per the note in 2.1.
3. **Adding `BURSAR` to `CARD_VIEWING_ROLES`** versus a narrower staff screen,
   per the note in 2.4.
4. **The takings report** is deliberately out of this phase and is filed as
